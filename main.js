...
window.onload = () => {
  ...
  map.setView(new L.LatLng(35.5, 100), 4);
  map.setMaxBounds([[16, 73], [55, 135]]);
  map.setMaxZoom(8);
  map.setMinZoom(3);
  ...
  if (window.configData) {
    if (configData.title) {
      ...
      document.getElementById('header').innerText = configData.title;
    }
    if (configData.tr) {
      L.control.attribution({position: 'topright', prefix: configData.tr}).addTo(map);
    }
    if (configData.bl) {
      L.control.attribution({ position: 'bottomleft', prefix: configData.bl }).addTo(map);
    }
    if (configData.br) {
      L.control.attribution({ position: 'bottomright', prefix: configData.br }).addTo(map);
    }
  }
  if (!window.classData || !window.configData || !configData.br) {
    L.control.attribution({ position: 'bottomright', prefix: '联系我免费创建班级地图<br/><a href="mailto: hullqin@outlook.com">hullqin@outlook.com</a>' }).addTo(map);
  }
  ...
  for (let i = 0; i < data.features.length; i++) {
    const name = data.features[i].properties.name_local;
    L.marker([data.features[i].properties.latitude, data.features[i].properties.longitude], {
      icon: L.divIcon({
        html: "<div class='map-province-divicon" +
          "'>" + name + "</div>", className: '', iconSize: L.point(name.length * 13, 18)
      }), interactive: false
    }).addTo(map);
  }
  map.addLayer(shape);
  ...
  function getCityData() {
    const cityChinese = ["甘肃", "青海", "广西", "贵州", "重庆", "北京", "福建", "安徽", "广东", "西藏", "新疆", "海南", "宁夏", "陕西", "山西", "湖北", "湖南", "四川", "云南", "河北", "河南", "辽宁", "山东", "天津", "江西", "江苏", "上海", "浙江", "吉林", "内蒙古", "黑龙江", "台湾"];
    const cityEnglish = ["Gansu", "Qinghai", "Guangxi", "Guizhou", "Chongqing", "Beijing", "Fujian", "Anhui", "Guangdong", "Xizang", "Xinjiang", "Hainan", "Ningxia", "Shaanxi", "Shanxi", "Hubei", "Hunan", "Sichuan", "Yunnan", "Hebei", "Henan", "Liaoning", "Shandong", "Tianjin", "Jiangxi", "Jiangsu", "Shanghai", "Zhejiang", "Jilin", "Inner Mongol", "Heilongjiang", "Taiwan"];
    const cityData = {
      "Gansu": 0,
      "Qinghai": 0,
      "Guangxi": 0,
    ...
    if (window.classData) {
      for (let i = 0; i < classData.length; i++) {
        if (escape(classData[i].city).indexOf("%u") >= 0) {
          for (let j = 0; j < cityChinese.length; j++) {
            if (classData[i].city.indexOf(cityChinese[j]) >= 0) {
              classData[i].city = cityEnglish[j];
              break;
            }
          }
        }
        eval("cityData['" + classData[i].city + "'] += 1;");
      }
    }
    return cityData;
  }
  ...
  function onEachFeature(feature, layer) {
    const popupContent = getContentByData(feature.properties.name);
    if (popupContent !== null) {
      const popup = layer.bindPopup(popupContent, { maxWidth: 200, minWidth: 100 });
      popup.on({
        popupopen: function () {
          const imgSrc = getRandomPicture(feature.properties.name);
          document.getElementById('food_' + feature.properties.name.replace(' ', '')).setAttribute('src', imgSrc);
          mapElement.style.backgroundImage = 'url(' + imgSrc + ')';
          layer._popup._updateLayout();
          layer._popup._updatePosition();
        },
        popupclose: function () {
          mapElement.style.backgroundImage = 'url("'+ getRandomBackground() +'")';
        }
      })
    }
    layer.on({
  ...
  function getContentByData(name) {
    let content = "<table><tr>";
    if (window.classData) {
      for (let i = 0; i < classData.length; i++) {
        if (classData[i].city === name) {
          content += "<td class='popup-table-name'>" + classData[i].name + "</td><td class='popup-table-university'>" + classData[i].university + "</td>";
          content += "</tr><tr>";
        }
      }
    }
    ...
    if (content.indexOf("td") < 0) {
      if (window.configData && configData.blank) {
        content = configData.blank;
      } else {
        content = ""
      }
    }
    content = "<img alt='food' width='100%' id='food_" + name.replace(' ', '') + "'/>" + content;
    return content;
  }
  ...
  function resetHighlight(e) {
    shape.resetStyle(e.target);
  }

  function getRandomPicture(_city) {
    const city = _city.toLowerCase().replace(' ', '');
    let max;
    switch (city) {
      case ("anhui"):
        max = 8;
    ...
    return window.publicPath + "img/food/" + city + "/" + num + ".jpg";
  }
  function getRandomBackground() {
    if (window.configData && configData.bg && configData.bg.length > 0) {
      const max = configData.bg.length;
      let num = Math.ceil(Math.random() * max);
      if (num < 1 || num > max) {
        num = 1;
      }
      num--;
      return window.publicPath + 'img/main' + window.location.pathname + '/' + configData.bg[num];
    }
    return window.publicPath + 'img/main/default.jpg';
  }
}